...
int main()
{
    ...
    sf::Shader mShader;
    mShader.loadFromFile("mandelbrot.glsl", sf::Shader::Fragment);


    sf::Vector2f zoomCenter(0.0, 0.f);
    float zoomSize = 4.f;
    ...
    while (window.isOpen())
    {
        ...
        shader.setUniform("u_mousePosition", mousePosition);

        mShader.setUniform("u_resolution", sf::Vector2f(Width, Height));
